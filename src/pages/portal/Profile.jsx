...
const Profile = () => {
  ...
  React.useEffect(() => {
    if (profile) {
      setFormData({
        fullName: profile.fullName || profile.contactName || profile.companyName || accountInfo?.name || '',
        phone: profile.phone || '',
        ...
        <div className="mt-6 rounded-lg bg-white/5 backdrop-blur-sm border border-white/10 p-4">
          ...
          <dl className="space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <dd className="text-white/90">{accountInfo?.email || profile?.email || 'Not available'}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-white/50">User ID</dt>
              <dd className="text-white/90 font-mono text-xs">{accountInfo?.id || 'Not available'}</dd>
            </div>
            <div className="flex justify-between">
